...
describe('APP User endpoint tests', () => {
  beforeEach((done) => {
    let userOne = new User({username: 'cep', firstName: 'Carla', password: createPasswordObject('hat')});
    let userTwo = new User({username: 'chip', firstName: 'Sam', password: createPasswordObject('bell')});
  ...
  it('can create user at POST /APP/user/signup', (done) => {
    let createUser = new User({username: 'pavo', firstName: 'Sarah', password: createPasswordObject('cute')});
    request(app)
  ...
  it('can log user in at POST /APP/user/login', (done) => {
    request(app)
      .post('/app/user/signup')
      .send({username: 'chip', password: createPasswordObject('bell')})
      .expect(res => {
        expect(201);
      }).end(done);
  });
...
describe('API User endpoint tests', () => {
  ...
  it('can view one user at POST /API/user/viewOne', (done) => {
    let createUser = new User({username: 'bubbahut', firstName: 'Bob', password: createPasswordObject('dingo')});
    createUser.save().then(() => {
      request(app)
        ...
        .expect(res => {
          expect(302);
          expect(res.body.data.firstName).to.equal('Bob');
        }).end(done);
    });
  });
  ...
  it('can view all users at GET /API/user/viewAll', (done) => {
    request(app)
      ...
      .expect(302)
      .expect(res => {
        expect(res.body.data[0].username).to.equal('cep');
      }).end(done);
  });
